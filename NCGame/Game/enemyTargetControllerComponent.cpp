...
void EnemyTargetControllerComponent::Update()
{
	...
	m_owner->GetTransform().rotation = Math::LerpDegrees(m_owner->GetTransform().rotation, rotation, 3.0f * dt);

	Vector2D force = Vector2D::Rotate(Vector2D::up, m_owner->GetTransform().rotation * Math::DegreesToRadians + Math::PI);
	//DEBUG_DRAW_LINE(m_owner->GetTransform().position, m_owner->GetTransform().position + force * 60.0f, Color::white);
